    ...
    <div ref="dragPreview" class="drag-preview-container" style="position: fixed; top: -9999px; left: -9999px; pointer-events: none; opacity: 0; visibility: hidden;">
      <div class="drag-preview-wrapper">
        ...
        <div class="tabs-container drag-preview-tabs-container">
          <div class="tab-item drag-preview-tab">
...
const handleDragStart = (event: DragEvent, tab: TabItem, zone: 'main' | 'additional') => {
  ...
  if (event.dataTransfer) {
    ...
    if (dragPreview.value) {
      ...
      const previewTab = dragPreview.value.querySelector('.drag-preview-tab') as HTMLElement
      const previewTabTitle = dragPreview.value.querySelector('.drag-preview-tab .tab-title') as HTMLElement
      const previewContent = dragPreview.value.querySelector('.drag-preview-content') as HTMLElement
      
      if (previewWrapper && previewTabsContainer && previewTab && previewTabTitle && previewContent) {
        // 设置tab标题
        previewTabTitle.textContent = tab.title
        
        // 检查是否是当前激活的tab
        const isActiveTab = (zone === 'main' && activeMainTab.value === tab.key) || 
                            (zone === 'additional' && activeAdditionalTab.value === tab.key)
        ...
        const yOffset = containerRect.top - previewWrapper.getBoundingClientRect().top + Math.min(event.offsetY, tabRect.height || 50)
...
const handleDragEnd = () => {
  ...
  if (dragPreview.value) {
    ...
    if (previewTab) {
      ...
      previewTab.style.alignItems = ''
      previewTab.style.gap = ''
      previewTab.style.padding = ''
      previewTab.style.background = ''
      previewTab.style.color = ''
      previewTab.style.borderRadius = ''
      previewTab.style.cursor = ''
      previewTab.style.userSelect = ''
      previewTab.style.transition = ''
      previewTab.style.boxShadow = ''
      previewTab.style.fontSize = ''
      previewTab.style.fontWeight = ''
    }
