<template>
  <div>
    ...
    <div>
      ...
      <h4>解决方案--在外出包装一个template组件进行遍历，在template中使用v-for,然后在内层做v-if的判断</h4>
    </div>
    ...
    <div>
      <h2>组件上使用 v-for</h2>
      <MyComponentList v-for="item in comList" :key="item.id" :value="item.name" :name="'这是名字:' + item.name + '-' + item.id" />
    </div>
